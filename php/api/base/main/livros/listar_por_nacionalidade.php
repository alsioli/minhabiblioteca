...
function GetMethod() {
    ...
    $sql = "
        SELECT
            ISNULL(NULLIF(TRIM(nacionalidade), ''), 'Não informado') AS nacionalidade,
            COUNT(*)                                                   AS total,
        ...
        GROUP BY ISNULL(NULLIF(TRIM(nacionalidade), ''), 'Não informado')
        ORDER BY total DESC
    ";
    ...
    try {
        ...
        $result_error = 'Erro ao listar por nacionalidade: ' . $e->getMessage();
    }
}
